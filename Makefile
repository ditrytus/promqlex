...
ANTLR4_ARGS = -Dlanguage=Go -o ../$(ANTLR4_OUTPUT_DIR) -package $(LOWERCASE_GRAMMAR_NAME) -no-listener -visitor
...
$(eval PARSER_OUTPUT := $(ANTLR4_OUTPUT_DIR)/$(LOWERCASE_GRAMMAR_NAME)_parser.go $(ANTLR4_OUTPUT_DIR)/$(LOWERCASE_GRAMMAR_NAME)parser_visitor.go parsers/$(LOWERCASE_GRAMMAR_NAME)parser_base_visitor.go)
